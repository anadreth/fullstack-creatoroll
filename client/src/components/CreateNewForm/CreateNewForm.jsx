...
const CreateNewForm = ({ fetchLink, type, iconList, getAll, naming }) => {
  ...
  const handleConfirm = () => {
    ...
    if (
      ...
      !formRef.current.values.shortDescription ||
      !formRef.current.values.iconPath
    ) {
      setError(true);
      return;
    }
  ...
  const createNew = async (values, onSubmitProps) => {
    setServerError(null);
    try {
      const savedNewResponse = await fetch(fetchLink, {
        method: "POST",
        headers: {
  ...
  const getDescription = async () => {
    ...
    try {
      const responseDescription = await fetch(
        "https://creato-roll-server.onrender.com/background/description",
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  return (
    ...
    >
          ...
          {({
            ...
            <LayoutGroup>
              <form onSubmit={handleSubmit}>
                <motion.div
                  className="bg-light absolute top-0 left-0 h-screen w-full p-3 flex flex-col justify-between items-center"
                  animate={{ opacity: [0, 1] }}
                ...
                >
                  ...
                  <div className="h-full md:h-4/6 flex flex-col justify-start items-center my-auto">
                    {iconList !== "" ? (
                      <motion.div
                        className="w-80 flex justify-between mb-3 items-center"
                        animate={{ opacity: [0, 1] }}
                        transition={{ duration: 0.1, delay: 0.1 }}
                      >
                        <select
                          className="shadow-md rounded-lg p-3 focus:outline-none text-opacity-60  text-dark"
                          name="iconPath"
                        ...
                        {values.iconPath && (
                          <motion.img
                            src={values.iconPath}
                            alt="icon"
                            className="m-auto aspect-square w-[40px] h-[45px]"
                            animate={{ scale: [0, 1] }}
                          />
                        )}
                        <button
                    ...
                    {errors.shortDescription && touched.shortDescription && (
                      <div className="text-red mb-3">
                        <Error message="One line description is required" />
                      </div>
                    )}
                    ...
                    {show && (
                      <motion.div
                        className="w-80 bg-red rounded-lg text-light shadow-md mb-3 p-3 text-center"
                        animate={{ scale: [0, 1] }}
                      >
                        {!loading ? (
                          <div>
                            ...
                            <button
                              ...
                              className="p-3 transition-all duration-150 rounded-lg shadow-md bg-orange hover:shadow-yogurt"
                            >
                              Generate Decription
                            </button>
                          </div>
                          ...
                          <div>
                            ...
                            <button
                              disabled
                              type="button"
                              className="rounded-lg w-44 p-3 mr-2 text-lg text-red bg-white shadow-md inline-flex justify-center items-center"
                            >
                              <Loading />
                            </button>
                          </div>
                    ...
                    {serverError && (
                      <div className="text-red pb-3 w-80 text-center">
                        <Error message={serverError} />
                      </div>
                    )}
                    {error && (
                      <div className="text-red pb-3 w-80 text-center">
                        <Error message="New Asset must have Icon, Title and One line description!" />
                      </div>
                    )}
                    <div className="flex justify-between items-center mt-auto w-80">
                      <button
                        className="bg-red hover:bg-dark-red transition-all duration-150 rounded-lg  text-light shadow-md w-32 p-3"
                        onClick={hideVisible}
                      >
                        Cancel
                      </button>
                      ...
                      <AnimatePresence>
                        {confirm && (
                          <motion.div
                            className="top-0 left-0 h-screen w-full absolute flex flex-col justify-center items-center"
                          ...
                          >
                            <div className="flex justify-center items-center flex-col rounded-lg w-80 md:w-96 border-red border-2 bg-red text-light p-3 ">
                              <p className="p-3 text-center">
                                This asset will be also available for other
                                users to use. <br /> Do you still wish to
                                continue?
                              </p>
                              <div className="w-full flex justify-around items-center m-3">
                                ...
                                {submitting ? (
                                  <p className="bg-orange hover:shadow-white hover:text-white hover:shadow-lg transition-all duration-150 rounded-lg text-light shadow-md w-32 p-3">
                                    Processing...
                                  </p>
                                ) : (
                                  <button
                                    className="bg-orange hover:shadow-white hover:text-white hover:shadow-lg transition-all duration-150 rounded-lg text-light shadow-md w-32 p-3"
                                    type="submit"
      ...
      <motion.button
        className="bg-red mb-3 transition-all duration-150 shadow-md text-light w-80 p-3 rounded-lg hover:bg-orange hover:text-white"
        onClick={showHidden}
        layout
      >
        Create New {naming}
