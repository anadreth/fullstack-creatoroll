...
const Background = ({ setGenerated, generated }) => {
  ...
  const getBackground = async () => {
    ...
    try {
      const responseBackground = await fetch(
        "https://creato-roll-server.onrender.com/background/get",
        {
          method: "POST",
          headers: {
  ...
  return (
    <div className="h-3/4 flex flex-col justify-center items-center">
      ...
      <motion.textarea
        className="focus:outline-none scrollbar-thumb-rounded-full scrollbar-track-rounded-full scrollbar-thin scrollbar-corner-red scrollbar-track-white scrollbar-thumb-red shadow-md h-1/2 p-3 w-80 resize-none overflow-scroll rounded-lg"
      ...
      />
      <motion.div
        className="bg-red p-3 m-3 shadow-md w-80 text-center rounded-lg"
        animate={{ opacity: [0, 1] }}
        transition={{ duration: 0.2, delay: 0.1 }}
      >
        <p className="text-light pb-3">
          {error ? error : "Out of ideas? Try generating!"}
        </p>
        {!generated && !loading ? (
          <button
            className="transition-all duration-150 hover:shadow-lg hover:shadow-light rounded-lg w-44 overflow-hidden shadow-md text-lg bg-orange text-light p-3"
          ...
          <button
            disabled
            type="button"
            className="w-44 p-3 mr-2 text-lg rounded-lg text-red bg-white shadow-md inline-flex justify-center items-center"
          >
            <Loading />
          </button>
        ) : (
          <button
            className="w-44 rounded-lg text-lg bg-white shadow-md text-red p-3"
            onClick={generateText}
          >
            Clear
          </button>
