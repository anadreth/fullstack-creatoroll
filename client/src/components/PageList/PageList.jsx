...
import { CreateNewForm, SearchBar, Loading, SearchIcon, Error} from './../index';
...
const PageList = ({type, getUrl, saveUrl, updateValue, title, iconList, error}) => {
    ...
    return (
        ...
        <div className="h-3/4 flex flex-col justify-center items-center">
            <motion.div className="flex justify-between items-center w-80 m-3"
                layout
                transition={{
                    opacity: { ease: "linear" },
                    layout: { duration: 0.1 }
            ...
            <motion.div className="scrollbar-thin scrollbar-corner-red scrollbar-track-white scrollbar-thumb-red h-96 w-80 shadow-md bg-white rounded-lg overflow-scroll mb-3 scrollbar-thumb-rounded-full scrollbar-track-rounded-full"
                layout
                transition={{
                    opacity: { ease: "linear" },
                    layout: { duration: 0.1 }
            ...
            <AnimatePresence>
                ...
                {serverError && <motion.div className="pb-3 px-3 text-red w-80 text-center" 
                    layout 
                    transition={{
                        opacity: { ease: "linear" },
                        layout: { duration: 0.1 }
                    ...
                    >
                    <Error message={serverError} /></motion.div>}

                {error ? <motion.div className="pb-3 px-3 text-red w-80 text-center" 
                    layout 
                    transition={{
                        opacity: { ease: "linear" },
                        layout: { duration: 0.1 }
                    ...
                    >
                    <Error message={`Your character must have a ${naming}`} /></motion.div>
                : <motion.p className="pb-3 px-3 text-red w-80 text-center"
                    layout 
                    transition={{
                        opacity: { ease: "linear" },
                        layout: { duration: 0.1 }
                    }}
                >Not finding what you want? <br /> Try creating your own!</motion.p>}
