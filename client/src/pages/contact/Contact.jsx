...
function Contact() {
  ...
  const onSubmit = async (values, onSubmitProps) => {
    ...
    send("service_cz9d09g", "template_8a1z9dc", values, "zIo1A7_py4nBPSuE-")
      .then((response) => {
        setLoading(false);
        setSubmitted(true);
      })
  ...
  return (
    <div className="bg-red text-light w-full h-auto ">
      ...
      <motion.h1
        className={`${
          screen.width < 1600 ? "hidden" : ""
        } font-seasons rotate-90 absolute top-80 -right-24 text-light opacity-10 text-[250px]`}
      ...
      {!submitted && !loading ? (
        ...
        <div className="bg-red text-light w-full h-screen flex justify-center items-center">
          <div className="w-3/4 text-center">
            <h2 className="p-3 text-5xl font-seasons">
              {error ? error : "MESSAGE SENT..."}
            </h2>
            <p className="font-poppins text-light">
              {error
                ? "Please try again later."
                : "Thank you! We will answer soon."}
            </p>
          </div>
        </div>
      )}
